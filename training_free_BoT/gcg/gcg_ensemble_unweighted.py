...
from training_free_BoT.gcg.baseline import RedTeamingMethod
        ...
        for step_i in tqdm(range(self.num_steps)):
            ...
            if verbose and (
                ...
                for i in range(len(losses)):
                    logging.info(
                        f"==> Model: {self.models[i]['model_name_or_path'].split('/')[-1]} | Loss {model_losses[i].min().item()}"
                    )
            ...
            if (
                ...
                and self.early_stop
                and all(behavior_losses[:, selected_id] < self.early_stop_min_loss)
            ):

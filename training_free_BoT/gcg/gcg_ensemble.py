...
from gcg.gcg_ray_actors import GCGModelWorker, get_nonascii_toks, sample_control

from utils.utils import is_thinking


# Create a minimal implementation of RedTeamingMethod to avoid circular imports
# This effectively creates a local version of what we need from baseline.py
class RedTeamingMethod:
    use_ray = False

    def generate_test_cases(self, behaviors, verbose=False):
        """Placeholder for generate_test_cases method"""
        raise NotImplementedError

    @staticmethod
    def get_output_file_path(save_dir, behavior_id, file_type, run_id=None):
        """Placeholder for get_output_file_path method"""
        raise NotImplementedError

    def save_test_cases(
        self, save_dir, test_cases, logs=None, method_config=None, run_id=None
    ):
        """Placeholder for save_test_cases method"""
        raise NotImplementedError

    @staticmethod
    def merge_test_cases(save_dir):
        """Placeholder for merge_test_cases method"""
        pass
...
class EnsembleGCG(RedTeamingMethod):
    ...
    def generate_test_cases(
        ...
        all_model_weights = [self.model_weights.copy()]
        all_model_losses = []
        completions = []
        # ========== run optimization ========== #
        for step_i in tqdm(range(self.num_steps)):
            ...
            # For logging
            avg_losses = weighted_model_losses
            ...
            for j in range(len(sampled_top_indices)):
                ...
                candidate_loss = avg_losses[j].item()
            ...
            if self.adaptive_weighting:
                ...
                all_model_weights.append(self.model_weights.copy())
            ...
            current_loss = avg_losses[selected_id].item()
            ...
            del grads, losses, model_losses, behavior_losses, avg_losses
            del weighted_model_losses
            torch.cuda.empty_cache()
            gc.collect()

        # Return the step logs for compatibility with the caller
        logs = step_logs
        return (
            optim_str,
            test_cases,

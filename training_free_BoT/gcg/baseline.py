...
import transformers
import vllm
from gcg.model_utils import load_model_and_tokenizer
...
class SingleBehaviorRedTeamingMethod(RedTeamingMethod):
    ...
    def generate_test_cases(self, behaviors, verbose=False):
        ...
        for behavior_dict in tqdm(behaviors, total=len(behaviors)):
            ...
            num_batches = int(
                np.ceil(self.num_test_cases_per_behavior / self.test_cases_batch_size)
            )
            ...
            for j in range(num_batches):
                ...
                if verbose:
                    print(
                        f"Generating batch {j+1}/{num_batches} with size {current_batch_size}"
                    )
                batch_test_cases, batch_logs = (
                    self.generate_test_cases_single_query_single_model(
                        behavior_dict, num_generate=current_batch_size, verbose=verbose
                    )
                )
                if isinstance(batch_test_cases, list) and (
                    len(batch_test_cases) == len(batch_logs)
                ):
    ...
    def generate_test_cases_single_query_single_model(
        self, behavior_dict, num_generate=1, verbose=False
    ):
        """
    ...
    def save_test_cases_single_behavior(
        ...
        test_cases_save_path = self.get_output_file_path(
            save_dir, behavior_id, "test_cases", run_id
        )
        logs_save_path = self.get_output_file_path(
            save_dir, behavior_id, "logs", run_id
        )
        method_config_save_path = self.get_output_file_path(
            save_dir, behavior_id, "method_config", run_id
        )
    ...
    def merge_test_cases(save_dir):
        ...
        behavior_ids = [
            d
            for d in os.listdir(
                os.path.join(save_dir, "test_cases_individual_behaviors")
            )
        ]
        ...
        if len(logs) == 0:
            ...
            with open(
                logs_save_path, "w"
            ) as f:  # Assuming you want to save logs in json format
                json.dump(logs, f, indent=4)
